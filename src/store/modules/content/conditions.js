...
const conditions_getters = {
	condition_services: (state) => {
		return state.condition_services;
	},
...
const conditions_actions = {
	...
	async fetch_condition({ commit, state, dispatch }, payload) {
		...
		let condition =
			state.cached_conditions[state.cached_urls[cache_key]] || state.cached_conditions[id];
		...
		if (!condition) {
			...
			try {
				...
				const description = condition.effects
					? `${condition.effects.join(" ").substring(0, maxLength).trim()}...`
					: "";
...
const conditions_mutations = {
	SET_CONDITION_SERVICES(state, payload) {
		state.condition_services = payload;
	},
	SET_CACHED_CONDITION(state, payload) {
		state.cached_conditions[payload["_id"]] = payload;
	},
	SET_CACHED_URL(state, { url, id, edition }) {
		state.cached_urls[`${edition || "5e"}:${url}`] = id;
	},
	SET_CACHED_CONDITIONS_LIST(state, { edition, conditions }) {
		state.cached_condition_lists[edition] = conditions;
	},
};
