...
const reminder_getters = {
	...
	reminder_count: (state) => {
		return state.reminder_count;
	},
	reminder_services: (state) => {
		return state.reminder_services;
	},
};
...
const reminder_actions = {
	 ...
	 */
	async get_reminders({ state, getters, rootGetters, dispatch, commit }) {
		const uid = rootGetters.user ? rootGetters.user.uid : undefined;
		let reminders = state.reminders ? state.reminders : undefined;

		if ((!reminders || getters.reminder_count > Object.keys(reminders).length) && uid) {
