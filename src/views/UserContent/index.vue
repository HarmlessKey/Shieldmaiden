		...
		<hk-card v-else class="banner">
			...
			<div v-else class="card-body">
				<div>
					...
					<h3 class="neutral-1"><strong>No active campaign</strong><br /></h3>
					<p class="neutral-3">Start your first adventure.</p>
				</div>

				<router-link to="campaigns" class="btn btn-sm bg-green"> Campaign overview </router-link>
			</div>
		</hk-card>
...
export default {
		...
		return {
			...
			dm_tabs: [
				...
				},
				{
					...
					label: "NPCs",
					icon: "fas fa-dragon",
					caption: "Your custom NPCs",
				},
				{
					path: "/content/spells",
					label: "Spells",
					icon: "fas fa-wand-magic",
					caption: "Your custom spells",
				},
				{
	...
	computed: {
		...mapGetters(["user", "tier", "userInfo", "content_count"]),
		active_campaign() {
			if (this.campaigns && this.userInfo) {
				if (this.userInfo && this.userInfo.active_campaign) {
					...
					if (campaign !== undefined) campaign.key = this.userInfo.active_campaign;

					return campaign;
				} else {
					const campaignKey = Object.keys(this.campaigns)[0];
					let campaign = this.campaigns[campaignKey];
					if (campaign !== undefined) campaign.key = campaignKey;
					return campaign;
				}
			} else {
				return undefined;
		...
		player_tabs() {
			...
			if (
				this.userInfo &&
				((this.userInfo.contribute && this.userInfo.contribute.includes("character-builder")) ||
					this.userInfo.admin)
			) {
				tabs.push({
	...
	methods: {
		...
		getBackground(campaign) {
			...
			if (campaign && campaign.hk_background)
				return require(`src/assets/_img/atmosphere/banner/${campaign.hk_background}-banner.jpg`);
