<template>
	<q-expansion-item class="request" :dark="$store.getters.theme === 'dark'" group="requests">
		<template #header>
			<q-item-section>
				<q-item-label caption>{{ entities[request.player].name }}</q-item-label>
				<q-item-label>
					{{ totalAmount }}
					<span :class="request.type === 'healing' ? 'green' : 'red'">{{ request.type }}</span>
				</q-item-label>
			</q-item-section>
			<q-item-label class="text-right neutral-3">
				...
				<q-item-label> Turn: {{ request.turn + 1 }} </q-item-label>
			</q-item-label>
		</template>
		<div class="accordion-body">
			...
			<template v-if="request.type === 'damage'">
				<div v-for="(result, index) in results" :key="`result-${index}`">
					...
					<div class="targets">
						<template v-for="(target, key) in result.targets" :key="`${key}-${i}`">
				...
				<div class="targets">
					<template v-for="(final, key) in final_results" :key="`final-${key}`">
						<div class="name truncate bg-neutral-8" v-if="entities[key]">
							{{ entities[key].name }}
						</div>
	...
	data() {
		return {
			...
			multipliers: [
				...
				{ multiplier: 0.5, name: "½", label: "Half damage" },
			...
			results.forEach((result) => {
				...
				for (let target of targets) {
					...
					for (const [key, defense] of Object.entries(resistances)) {
						if (
							this.entities[target][defense] &&
							this.entities[target][defense].includes(result.damage_type)
						) {
