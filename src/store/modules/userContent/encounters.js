...
const encounter_mutations = {
	...
	SET_ENTITY_PROP(state, { uid, campaignId, encounterId, entityId, property, value }) {
		if (value === null) {
			delete state.cached_encounters[uid][campaignId][encounterId].entities[entityId][property];
		} else {
	...
	SET_TRANSFORMED_PROP(state, { uid, campaignId, encounterId, entityId, property, value }) {
		if (value === null) {
			delete state.cached_encounters[uid][campaignId][encounterId].entities[entityId].transformed[
				property
			];
		} else {
	...
	UPDATE_LIMITED_USES(state, { uid, campaignId, encounterId, key, category, index, value }) {
		const entity = state.cached_encounters[uid][campaignId][encounterId].entities[key];
		if (entity.limited_uses) {
			if (entity.limited_uses[category]) {
				entity.limited_uses[category][index] = value;
			} else {
				entity.limited_uses[category] = { [index]: value };
			}
		} else {
	...
	SET_REMINDER(state, { uid, campaignId, encounterId, entity, key, reminder }) {
		const target = state.cached_encounters[uid][campaignId][encounterId].entities[entity];
		if (target.reminders) {
			target.reminders[key] = reminder;
		} else {
			target.reminders = { [key]: reminder };
		}
	},
	SET_LOOT(state, { uid, campaignId, encounterId, id, item }) {
		if (
			...
			const encounter = state.cached_encounters[uid][campaignId][encounterId];
			if (encounter.loot) {
				encounter.loot[id] = item;
			} else {
				encounter.loot = { [id]: item };
			}
		}
	...
	REMOVE_ITEM_LINK(state, { uid, campaignId, encounterId, parent_item }) {
		if (
			...
			delete state.cached_encounters[uid][campaignId][encounterId].loot[parent_item].linked_item;
		}
	},
	...
	SET_ENTITY_SETTING(state, { uid, campaignId, encounterId, entityId, key, value }) {
		const entity = state.cached_encounters[uid][campaignId][encounterId].entities[entityId];
		if (entity.settings) {
