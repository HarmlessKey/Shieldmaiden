...
const track_campaign_actions = {
	...
	async get_campaigns({ state, dispatch, commit }, uid) {
		...
		if (!campaigns) {
			try {
				SEARCH_CAMPAIGNS_REF.child(`${uid}/results`)
...
const track_campaign_mutations = {
	...
	SET_USER(state, { uid, user }) {
		state.users[uid] = user;
	},
	SET_USER_SETTINGS(state, { uid, settings }) {
		state.users[uid].settings = settings;
	},
	SET_USER_LIVE(state, { uid, live }) {
		state.users[uid].live = live;
	},
	SET_CAMPAIGNS(state, { uid, campaigns }) {
		state.track_search_campaigns[uid] = campaigns;
	},
