<template>
	<div class="meters">
		...
		<template
			v-if="Object.keys(_meters['damage']).length > 0 || Object.keys(_meters['healing']).length > 0"
		>
								...
								>
									<div class="info">
										...
										<span class="numbers">
											...
											>
												<template v-if="entity.meters[subtype] < 10000">{{
													entity.meters[subtype]
												}}</template>
												<template v-else>{{ $numeral(entity.meters[subtype], "0.0a") }}</template>
											</span>
											<template v-if="entity.meters[over]">
												(<template v-if="entity.meters[over] < 10000"
													>{{ entity.meters[over] }}
												</template>
...
export default {
	...
	props: ["entities", "campaign", "players", "npcs", "displaySettings"],
	data() {
		return {
