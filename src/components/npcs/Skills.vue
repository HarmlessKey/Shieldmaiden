			...
			<template v-slot:header>
				<div class="card-header d-flex justify-content-between">
					...
					<a class="btn btn-sm bg-neutral-5" @click="setDialog()">
						...
						<q-tooltip anchor="top middle" self="center middle"> Skill modifiers </q-tooltip>
					</a>
				</div>
			</template>
						...
						>
							...
							>
								<template v-slot:append>
									<i
										aria-hidden="true"
										class="fas fa-check green saved"
										v-if="saved.includes(key)"
										@animationend="saved.splice(saved.indexOf(key), 1)"
									/>
								</template>
							</q-input>
						</ValidationProvider>
...
export default {
	...
	methods: {
		...
		skillModifier(skill, key) {
			...
			const proficiency = this.npc.challenge_rating
				? this.monster_challenge_rating[this.npc.challenge_rating].proficiency
				: 0;
			const bonus =
				this.npc.skill_modifiers && this.npc.skill_modifiers[key]
					? this.npc.skill_modifiers[key]
					: 0;
