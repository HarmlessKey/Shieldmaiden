				...
				<hk-card :min-width="300" no-margin>
					<template v-slot:header>
						<div class="card-header d-flex justify-content-between">
							<span> {{ modifier[".key"] ? "Edit" : "New" }} modifier </span>
					...
					<div class="card-body">
						...
						<div v-if="!scaling">
							...
							<small class="d-block mt-1" v-if="modifier.type"
								><strong>{{ modifier.type.capitalize() }}</strong
								>: {{ type_info[modifier.type] }}</small
							>
							...
							<template v-if="modifier.type">
								...
								<div
									...
									v-if="['ability', 'saving_throw'].includes(modifier.target)"
								>
									<q-select
										dark
										filled
								...
								<div class="form-item mb-3" v-if="['bonus', 'set'].includes(modifier.type)">
									...
									<p
										...
										v-if="
											modifier.scaling &&
											modifier.scaling.scale &&
											modifier.scaling.scale.size &&
											modifier.scaling.scale.value
										"
									>
						...
						<!-- SCALING -->
						<div v-else>
							<a @click="scaling = false"
								><i class="fas fa-chevron-left" aria-hidden="true" /> Edit modifier</a
							>
							<p class="mt-3">
							...
							<div v-if="modifier.scaling.type === 'scale'" class="form-item mb-3">
								...
								<p
									v-if="modifier.scaling.scale.size && modifier.scaling.scale.value"
									v-html="scalingText()"
								/>
							</div>

							<!-- STEPS -->
							<div v-if="modifier.scaling.type === 'steps'"></div>
						</div>
					</div>
		...
		modifier_targets() {
			return [
				...
				},
				{
					value: "hp",
					label: "Hit points",
					disable: ["ability", "proficiency", "expertise", "advantage", "disadvantage"].includes(
						this.modifier.type
					),
				},
				{
					value: "armor",
					label: "Armor",
					disable: ["bonus", "set", "ability", "expertise", "advantage", "disadvantage"].includes(
						this.modifier.type
					),
				},
				{
