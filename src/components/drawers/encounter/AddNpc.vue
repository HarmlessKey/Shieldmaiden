...
export default {
	...
	methods: {
		...mapActions(["setDrawer", "add_entity_demo", "add_player", "add_npc"]),
		...
		set({ result, id, resource }) {
			...
			this.entity.edition = result.edition || default_edition;
		...
		isValid(key) {
			return (
				(this.selectedPlayers.includes(key) && this.playerInitiative[key] > 0) ||
