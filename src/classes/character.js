...
export class Character {
	...
	constructor(character) {
		...
		this.class.classes =
			character.class && character.class.classes
	...
	// RACE
	get race() {
		const race = JSON.parse(JSON.stringify(this._race));
	...
	set traits(value) {
		this._race.traits = value;
	}

	add_trait() {
		if (!this._race.traits) this._race.traits = [];
	...
	total_class_hp(classIndex, con) {
		...
		for (let level = 1; level <= Class.level; level++) {
			if (this.hit_point_type === "rolled") {
				...
				const rolled =
					classIndex == 0 && level === 1 ? Class.hit_dice : Class.rolled_hit_points[level] || 0;
				...
				// For the main class set a full hit die as starting hit points (php 12)
				const average =
					classIndex == 0 && level === 1 ? Class.hit_dice : calc_dice_average(hit_dice);
	...
	get proficiencies() {
		...
		for (const classIndex in this.classes) {
			...
			for (const type of types) {
				proficiencies[classIndex][type] =
					this.all_modifiers.filter((mod) => {
			...
			? modifiers.filter((modifier) => {
					if (modifier && modifier.origin) {
						...
						return !(
							origin[0] === "class" &&
							origin[1] == classIndex &&
							parseInt(origin[2]) > parseInt(Class.level)
						);
					}
			  })
			: [];
...
export class ComputedCharacter {
	...
	compute_character(character) {
		...
		for (const [key, value] of Object.entries(base_character.classes)) {
			if (value.casting_ability) {
				this.classes[key].spell_attack =
					this.proficiency + calc_mod(this.abilities[value.casting_ability]);
				this.classes[key].spell_save_dc =
					8 + this.proficiency + calc_mod(this.abilities[value.casting_ability]);
			}
		}
		...
		character.classes.forEach((value, index) => {
			...
			if (value.caster_type) {
				...
				if (character.classes[index].spells_known) {
					this.classes[index].spells_known = character.classes[index].spells_known.spells
						? character.classes[index].spells_known.spells[level] || 0
						: 0;
					this.classes[index].cantrips_known = character.classes[index].spells_known.cantrips
						? character.classes[index].spells_known.cantrips[level] || 0
						: 0;
				} else {
	...
	_compute_equipment(character) {
		...
		if (
			(this.armor && !proficiencies.armor.includes(this.armor.armor_type)) ||
			(this.shield && !proficiencies.armor.includes("shield"))
		) {
				...
				for (const subtarget of modifier.subtarget) {
					if (ability === subtarget) {
						if (
							...
							!this.saving_throws.proficiencies.includes(ability)
						) {
							this.saving_throws.proficiencies.push(ability);
						} else {
							let value = this.saving_throws.bonuses[ability]
								? this.saving_throws.bonuses[ability]
								: 0;
	...
	_add_modifier(character, value, modifier) {
		...
		if (modifier.scaling) {
			if (modifier.scaling.type === "scale" && modifier.scaling.scale) {
				...
				const starting_level =
					modifier.origin.split(".")[0] === "class"
						? modifier.origin.split(".")[2]
						: modifier.scaling.start;
				const current_level =
					modifier.origin.split(".")[0] === "class" ? this.classes[classIndex].level : this.level;

				//Calculate the increase based on starting level, character-/class-level and the scale
				const increase = parseInt(
					Math.floor((current_level - starting_level) / modifier.scaling.scale.size)
				);
		...
		if (modifier.type === "ability") {
			newValue = Math.floor(
				newValue + modifier_multiplier * calc_mod(this.abilities[modifier.ability_modifier])
			);
		}
