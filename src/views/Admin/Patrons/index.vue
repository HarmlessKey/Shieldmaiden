		...
		<hk-card v-if="!$route.params.id">
			<div class="card-header">
				<h1 class="d-flex justify-content-between">
					<span>
						<i aria-hidden="true" class="fab fa-patreon"></i> Patrons (
						{{ Object.keys(patrons).length }} )
					</span>
				</h1>
					...
					<div class="table-responsive">
						...
						>
							...
							<template v-slot:email="data">
								<router-link :to="'/admin/patrons/' + data.row['.key']">{{
									data.item
								}}</router-link>
							</template>
							...
							<template v-slot:last_charge_status="data">
								<span>
									<i
										aria-hidden="true"
										:class="{
											'green fas fa-check': data.item == 'Paid',
											'red fas fa-times': data.item == 'Declined',
										}"
									>
									</i>
								</span>
