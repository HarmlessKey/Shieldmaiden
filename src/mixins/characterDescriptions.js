		...
		replaceDescriptionStats(description, Class = undefined) {
			if(description) {
				for(const replace of this.replaceStats) {
					...
					description = description.replace(regex, `<b title="${replace.description}">${replace_value}</b>`);
				}
				...
				if(Class) {
					...
					description = description.replace(level_regex, `<b title="${Class.class} level">${level_replace_value}</b>`);

					//Spell attack
					const attack_regex = /\[spell_attack\]/g;
					const attack_replace_value = Class.spell_attack || "spell attack modifier";
					description = description.replace(attack_regex, `<b title="Spell attack modifier">${attack_replace_value}</b>`);

					//Spell save DC
					const save_regex = /\[spell_save_dc\]/g;
					const save_replace_value = Class.spell_save_dc || "spell save DC";
					description = description.replace(save_regex, `<b title="Spell save DC">${save_replace_value}</b>`);
				}
			}
