			...
			<template v-if="!loading">
				<li v-for="(notification, key) in _notifications" :key="key">
					...
					<b-collapse :id="'details' + key" class="mt-1">
						<ul>
							<li><span>Entitled Amount:</span> <span>{{ notification.attributes.currently_entitled_amount_cents }}</span></li>
							...
							<li><span>Lifetime support:</span> <span>{{ notification.attributes.lifetime_support_cents }}</span></li>
							...
							<li><span>Will pay:</span> <span>{{ notification.attributes.will_pay_amount_cents }}</span></li>
							<li>
								<span>Tier:</span> 
