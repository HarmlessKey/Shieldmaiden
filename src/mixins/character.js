...
export const characterMixin = {
	...
	methods: {
		...
		compute_character(character, origin) {
			...
			let computed_hp = base_character.class.classes[0].hit_dice
				? base_character.class.classes[0].hit_dice
				: 0;
			...
			base_character.class.classes.forEach((value, index) => {
				...
				if (hit_point_type === "rolled" && value.rolled_hit_points) {
					...
					computed_hp =
						index == 0
							? computed_hp + (level - 1) * hit_dice[0].average
							: computed_hp + level * hit_dice[0].average;
				}
				...
				if (value.caster_type) {
					...
					classes[index].spells_known =
						base_character.class.classes[index].spells_known.spells[level] || 0;
					classes[index].cantrips_known =
						base_character.class.classes[index].spells_known.cantrips[level] || 0;
				}
			});
			...
			for (const [classKey, value] of Object.entries(classes)) {
				for (const index in modifiers) {
					...
					if (
						origin[0] === "class" &&
						origin[1] == classKey &&
						parseInt(origin[2]) > parseInt(value.level)
					) {
			...
			if (
				(armor && !proficiencies.armor.includes(armor.armor_type)) ||
				(shield && !proficiencies.armor.includes("shield"))
			) {
			...
			for (const [key, value] of Object.entries(base_character.class.classes)) {
				if (value.casting_ability) {
					classes[key].spell_attack =
						proficiency + this.calcMod(ability_scores[value.casting_ability]);
					classes[key].spell_save_dc =
						8 + proficiency + this.calcMod(ability_scores[value.casting_ability]);
				}
			}
			...
			if (base_character.race) {
				let speed = base_character.race.walking_speed
					? parseInt(base_character.race.walking_speed)
					: 0;
				...
				for (const modifier of this.modifierFilter(modifiers, "skill")) {
					...
					if (skill === modifier.subtarget) {
						if (modifier.type === "proficiency") {
							...
							skills.bonuses[skill] = this.addModifier(
								value,
								modifier,
								proficiency,
								ability_scores,
				...
				for (const modifier of this.modifierFilter(modifiers, "saving_throw")) {
					...
					if (ability === modifier.subtarget) {
						if (modifier.type === "proficiency") {
							...
							saving_throws.bonuses[ability] = this.addModifier(
								value,
								modifier,
								proficiency,
								ability_scores,
				...
				if (modifier.scaling_type === "scale") {
					...
					const starting_level =
						modifier.origin.split(".")[0] === "class"
							...
							: modifier.scaling_start;
					const current_level =
						modifier.origin.split(".")[0] === "class" ? classes[classKey].level : character_level;

					//Calculate the increase based on starting level, character-/class-level and the scale
					const increase = parseInt(
						Math.floor((current_level - starting_level) / modifier.scale_size)
					);
				...
				if (modifier.subtarget) {
					if (this.advantage_disadvantage[modifier.target]) {
						if (this.advantage_disadvantage[modifier.target][modifier.subtarget]) {
							this.advantage_disadvantage[modifier.target][modifier.subtarget][modifier.type] =
								true;
						} else {
