...
import validation, { extend } from "src/plugins/validation";

export default async ({ app }) => {
	app.use(validation);

	// Message overrides for the built-in rules (src/plugins/validation/rules.js).
	extend("required", { message: "{_field_} is required" });
	extend("max", { message: "{_field_} must be {length} or less" });
	extend("max_value", { message: "{_field_} must be {max} or less" });
	extend("min", { message: "{_field_} can't be under {length} " });
	extend("min_value", { message: "{_field_} can't be under {min} " });
	extend("alpha_dash", {
	...
	extend("variable_check", {
		...
		validate: (value, variables) => {
			...
			for (let v of text_vars) {
				...
				if (!variables[0] || !Object.keys(variables[0]).includes(stripped)) return false;
			}
			return true;
		},
	});
