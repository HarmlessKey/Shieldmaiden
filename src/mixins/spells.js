...
export const spells = {
	...
	methods: {
		rollSpell(spell, castLevel, casterLevel, toHitModifier) {
			...
			for(let action of actions) {
				...
				if(type === 'Melee Weapon' || type === 'Ranged Weapon' || type === 'Spell Attack') {
					...
					console.log('To Hit: ', returnRoll.toHit)
				}
				if(type === 'Spell Save') {
				...
				for(let modifier of action.modifiers) {
					...
					let scaledModifier = this.__levelScaling__(tiers, castLevel, spellLevel, scaleType);
					
					//Roll the modifier
					let modifierRoll = this.rollD(dice_type, dice_count, fixed_val);
					console.log('Modifier: ', modifierRoll);

					if(scaledModifier) {
						scaledRoll = this.rollD(scaledModifier.dice_type, scaledModifier.dice_count, scaledModifier.fixed_val);
						console.log('Scaled: ', scaledRoll);
					}
		...
		__levelScaling__(tiers, castLevel, spellLevel, scaleType) {
