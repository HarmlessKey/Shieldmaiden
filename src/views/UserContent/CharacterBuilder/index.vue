<template>
	<hk-card>
		...
		<div class="card-body">
			<h3>
				Thank you <strong>{{ userInfo.username }}</strong> for helping us test our Character
				Builder!
			</h3>
			...
			<p>
				...
				If you find a <strong>bug</strong>, please report it on our Discord in the designated
				channel
				<a
					href="https://discord.com/channels/654675574488563714/982257934153166858"
					target="_blank"
					rel="noopener"
					>#character-builder</a
				>.
			...
			<p>
				During our closed alpha phase we might at any time delete all characters of all users. We
				will send a notification on Discord whenever we do this.
			</p>
					...
					<template v-slot:body="props">
						<q-tr :props="props">
							<q-td
								...
								:style="
									col.name === 'avatar' && col.value ? `background-image: url('${col.value}')` : ''
								"
							>
...
export default {
	...
	preFetch({ store, redirect }) {
		...
		if (
			...
			!store.getters.userInfo.contribute ||
			!store.getters.userInfo.contribute.includes("character-builder")
		) {
			redirect("/");
		}
	},
	...
	computed: {
		...mapGetters(["tier", "overencumbered", "userInfo"]),
