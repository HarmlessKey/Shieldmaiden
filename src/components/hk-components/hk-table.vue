		...
		<div class="filters" v-if="search !== undefined">
			...
			>
				<template v-slot:append>
					<q-icon name="fas fa-search" size="xs" class="pointer" @click="searchData()" />
				</template>
			</q-input>
			<div
				v-if="searched !== undefined && searched !== ''"
				class="green result-count"
				:class="{ red: Object.keys(dataItems).length === 0 }"
			>
				{{ Object.keys(dataItems).length }} results for {{ searched }}
			</div>
		</div>
		<!-- TABLE -->
		<div v-if="!loading" class="hk-table" :style="{ 'grid-template-columns': templateColumns }">
			<!-- By default, show a header -->
			<template v-if="showHeader">
				<div v-if="collapse" class="hk-table-column hk-table-header" :key="`collapse-header`">
						...
						>
							...
							<span class="sort">
								<i
									...
									:class="{ blue: !reverse && sortedBy === key }"
								></i>
								<i
									aria-hidden="true"
