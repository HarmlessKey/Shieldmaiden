<template>
	<div>
		<div class="type d-flex justify-content-between">
			<button
				class="btn bg-neutral-4 mb-3"
				:class="{ 'bg-red': type === 'damage' }"
				@click="type = 'damage'"
			>
...
export default {
	...
	methods: {
		...
		sendRequest() {
			...
			db.ref(`encounters/${this.dmId}/${this.campaignId}/${this.encounter.key}/requests`).push(
				request
			);
