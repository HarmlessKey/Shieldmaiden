  ...
  async fetch_monster({ commit, state, dispatch}, id) {
    ...
    if(!monster) {
      ...
      try {
        ...
        monster.meta.description += monster.walk_speed ? `Speed: ${monster.walk_speed}ft.` : "Speed: 0ft.";
        ...
        for(const ability of ["strength", "dexterity", "constitution", "intelligence", "wisdom", "charisma"]) {
          monster.meta.description += ` ${ability.substring(0, 3).toUpperCase()}: ${monster[ability]}.`;
        }
