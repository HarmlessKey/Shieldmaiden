	...
	<div>
		<div class="encounter_overview" :class="{ show: showOverview }">
			...
			<hk-card v-if="demo || (tier && tier.name === 'Free')" class="background-effects">
				<video 
					slot="image"
					class="video" src="~assets/_vid/background-effects-rain.mp4" 
					muted autoplay playsinline alt="Background Effects Rain" loop
				/>
				<div class="p-3 text-center">
					...
					<q-btn to="/weather-demo" color="primary" label="Check out all effects" no-caps class="full-width" />
				</div>
			</hk-card>
		</div>
		<div class="toggle bg-green" :class="{ show: showOverview }"  @click="showOverview = !showOverview">
