<template>
	<div v-if="log">
		...
		<transition-group
			v-if="entities && Object.keys(log).length > 0"
			tag="ul"
			name="log"
			enter-active-class="anitmated slideInDown"
		>
			<li v-for="(item, key) in log" :key="`item-${key}`">
				...
				<div class="d-flex justify-between">
					...
					<small v-if="item.multiplier && item.multiplier !== 1" class="blue"
						><strong>
				...
				<div v-if="item.actions">
					<span v-for="(action, index) in item.actions" :key="`action-${key}-${index}`">
						...
						<!-- Rolls -->
						<span
							v-for="(roll, roll_index) in action.rolls"
							:key="`roll-${key}-${index}-${roll_index}`"
						>
							<span
								:class="
