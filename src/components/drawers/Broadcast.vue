<template>
	<div>
		...
		/>
		<span class="live mb-3" :class="{ active: broadcast.live === campaign_id }">
		...
		<p v-else>
			When you're live, your players can see the initiative list of your active encounter and you
			can choose to show them your rolls there as well. Your encounters can be followed with your
			<a @click="setDrawer({ show: true, type: 'PlayerLink' })">public initiative link.</a>
		</p>
		...
		>
			...
			<template v-slot:append>
				<q-icon
					v-if="shares.length > 0 && !broadcast.live"
					name="close"
					@click.stop="sharesCleared"
		...
		<a
			...
			@click="private_campaign ? null : live()"
			:class="{ disabled: private_campaign }"
		>
...
export default {
	...
	methods: {
		...mapActions(["setLive", "setLiveShares", "setDrawer"]),
